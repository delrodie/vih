...
class CommentaireController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        $rapportId = $request->get('rapport'); //dump($rapportId);die();
        $form = $this->createForm('AppBundle\Form\CommentaireType', $commentaire, ['rapport'=>$rapportId]);
        $form->handleRequest($request);
        $em = $this->getDoctrine()->getManager();

        $rapport = $em->getRepository('AppBundle:Rapport')->findOneBy(['id'=>$rapportId]);
        $commentaires = $em->getRepository('AppBundle:Commentaire')->findBy(['rapport'=>$rapportId]);

        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();//dump($commentaire);die();
